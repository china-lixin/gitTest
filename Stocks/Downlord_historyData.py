...
data_dir = '/home/user/share/'  # 下载数据的存放路径
    ...
    if is_open_day(str_date):
        ...
        if not os.path.exists(file):
            try:
                ...
                print
                'Get tick data error: symbol: ' + symbol + ', date: ' + str_date + ', sleep time is: ' + str(sleep_time)
                return res
            else:
                d.to_csv(file)
                ...
                print
                "Successfully download and save file: " + file + ', sleep time is: ' + str(sleep_time)
                return res
        else:
            print
            "Data already downloaded before, skip " + file
            res = False
            return res
